const BACKEND_URL = 'https://script.google.com/macros/s/AKfycbz4hbYqmel2bZGiy5iOy01X0TIFBrQecHKrRZ1HcJTOM3cu9GByyak9sJ8o-IWZG0KzRQ/exec';

let registrosBaseA = [];
let baseA_ID = null;

async function cargarBaseADesdeInput() {
    const url = document.getElementById("basea-url").value.trim();
    const match = url.match(/\/d\/([a-zA-Z0-9-_]+)/);
    if (!match || !match[1]) {
        alert("URL inválida. Asegúrese de pegar un enlace de Google Sheets como: https://docs.google.com/spreadsheets/d/ID/edit");
        return;
    }
    ...
    try {
        ...
        if (json.success) {
            ...
            alert("BaseA cargada correctamente.");
            actualizarVista();
        } else {
            alert("No se pudo cargar BaseA: " + json.mensaje);
        }
    } catch (error) {
        console.error("Error al cargar BaseA:", error);
        alert("Error de red al intentar cargar BaseA.");
    }
}
...
function mostrarModalItem(itemId) {
    ...
    if (registrosBaseA.length === 0) {
        alert("Debe cargar primero la BaseA con un enlace válido.");
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    crearGrilla(); // ya no se carga BaseA automáticamente
});
